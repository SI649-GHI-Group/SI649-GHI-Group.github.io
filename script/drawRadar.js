...
// let GLOBAL_YEAR=selectedYear
let GLOBAL_REGION='SSAfrican'

// let SSAfricanCountries = ["Burkina Faso","Burundi","Central African Republic","Djibouti","Guinea","Lesotho","Madagascar","Mali","Mauritania","Niger","Nigeria","Sierra Leone","South Africa","Zambia"]
...
const readCSV=(fileName)=>{
	let groupedtempdata=[]
	d3.csv(`assets/data/${fileName}.csv`, function(d) {
		tempdata= d;
...
const simulateData=(data)=>{
	...
	countyName.forEach((cty)=>{
		...
		// console.log("linearRegressionR",linearRegressionR)
		// console.log("cty",cty)
		// console.log("singleCountry",singleCountry)
		// let maxYear=d3.max(singleCountry,(cy)=>{return cy.Year})
		// let minYear=d3.min(singleCountry,(cy)=>{return cy.Year})
		// let minValue=singleCountry.filter((d)=>{return d.Year===minYear})[0][keys[3]]
		// let maxValue=singleCountry.filter((d)=>{return d.Year===maxYear})[0][keys[3]]
		// let yearGap=(maxValue-minValue)/(maxYear-minYear)
		// let catalog=[keys[3]
		// console.log("singleCountry",singleCountry)
...
const updateRadarData=(results)=>{
	// groupedtempdata=groupDataByRegin(tempSimulatedData)
	groupedStuntingD=groupDataByRegin(results[3])
...
const loadRadarData=(year = 1992)=> {
	urls =  ['Underweight','Wasting','Mortality','Stunting','Undernourishment']
...
const loadYearData=(year,region)=>{
	...
	let reginFocusStuntingD=groupedStuntingD.filter((d)=>{
																							return d.key===region
																						})[0]
																						.values
																						.filter((i)=>{
	...
	radardata=commonElements.map((cntry)=>{
		...
		countryradarTemp.axes=[
			{axis: 'Underweight', value: reginFocusUndernourishmentD.filter(d=>d.Entity===cntry)[0].Undernourishment, predict:reginFocusUndernourishmentD.filter(d=>d.Entity===cntry)[0].Predict },
			...
			{axis: 'Mortality', value: reginFocusWeightD.filter(d=>d.Entity===cntry)[0].Underweight, predict:reginFocusWeightD.filter(d=>d.Entity===cntry)[0].Predict},
			...
			{axis: 'Undernourishment', value: reginFocusMortalityD.filter(d=>d.Entity===cntry)[0].Mortality, predict:reginFocusMortalityD.filter(d=>d.Entity===cntry)[0].Predict}]
...
const drawRadarChart=(yearControl,regionControl)=>{
	...
	var radarChartOptions2 = {
	  ...
	  color: d3.scaleOrdinal().range([ "#ff6600"]),
		format: '.0f',
		legend: { title: '', translateX: 110, translateY: 20 },
		unit: '%'
	};

	let chartsFrame=document.querySelector("#charts")
	// console.log(document.querySelector(".single-chart"))
	// if(document.querySelector(".single-chart")){
	//
	// }
	data.forEach((d)=>{
		// console.log("d",d)
